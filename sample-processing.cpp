...
void SampleProcessing::createNew(std::string data, std::string fileName, std::string dirName)
{
	...
	LOG_INFO("Creating of %s", fullName.c_str());
	std::ofstream fout(fullName, std::ios::binary);

	if (fout.is_open())
	{
		LOG_DEBUG("Trying write %d bytes", data.size());
		fout.seekp(0, std::ios::beg);
		size_t previousPos = fout.tellp();
		fout.write(&data[0], data.size());
		size_t wrritenBytes = fout.tellp() - previousPos; 
		LOG_DEBUG("%d bytes was written", wrritenBytes);
	}
	else
	{
		LOG_ERROR("Failed to create %s: %s", fullName.c_str(), std::strerror(errno));
		exit(EXIT_FAILURE);
	}
	LOG_INFO("%s was created successfully", fullName.c_str());
}

std::string SampleProcessing::getBytes(std::string fileName, std::string dirName)
{
	std::string fullName = dirName + "/" + fileName;
	LOG_INFO("Start getting bytes from %s", fullName.c_str());
	...
	if (fin.is_open())
	{
		...
		LOG_DEBUG("%d bytes was read", countOfReadBytes);
	...
	{
		LOG_ERROR("Failed to open %s: %s", fullName.c_str(), std::strerror(errno));
		exit(EXIT_FAILURE);
	}
